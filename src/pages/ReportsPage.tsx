...
export const ReportsPage: React.FC = () => {
  ...
  const filteredTransactions = transactions.filter(t => {
    ...
    const isDateMatch = isWithinInterval(t.timestamp, { 
  ...
  const renderStockReport = () => {
    const totalStockValue = items.reduce((sum, i) => sum + i.currentStock * i.basePrice, 0);
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <SummaryCard label="Low Stock Items" value={items.filter(i => i.currentStock <= i.reorderThreshold).length.toString()} icon={Filter} color="red" />
        </div>
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <SummaryCard label="Active Customers" value={customers.filter(c => c.balance > 0).length.toString()} icon={Users} color="blue" />
        </div>
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100">
            ...
            <tbody className="divide-y divide-gray-50">
              {customers.filter(c => c.balance > 0).map(c => (
                <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="p-4 text-sm text-gray-600">{c.shopName || 'N/A'}</td>
                  <td className="p-4 text-sm font-bold text-red-600 text-right">{c.balance.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
